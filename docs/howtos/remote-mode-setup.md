# How to Setup and Use Remote Mode (oryn-r)

**Remote Mode** allows Oryn to control a standard Chrome-based browser via a lightweight WebSocket extension. This is ideal for visual debugging and development.
...
Build the `oryn-r` binary using Cargo:

```bash
cargo build -p oryn-r
```
...
4.  Select the `extension/` directory from the root of the Oryn project:
    ```
    /path/to/oryn/extension/
    ```
5.  You should see "Oryn Agent" in your extensions list.
...
Run the `oryn-r` server. By default, it listens on port `9001`.

```bash
cargo run -p oryn-r
# Or with a custom port:
# cargo run -p oryn-r -- --port 9001
```

You should see output indicating the server has started:
```
Starting Oryn Remote Backend on port 9001...
...
2.  Look for "Connected to Oryn Server" in the console.
3.  The server terminal logs might also show a new peer connection.

## Step 5: Execute Commands

You can now type [Intent Language](../SPEC-INTENT-LANGUAGE.md) commands directly into the `oryn-r` CLI.
...
- **"WebSocket error" in Extension**: Ensure `oryn-r` is running. Check if port 9001 is available.
