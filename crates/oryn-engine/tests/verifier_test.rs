use oryn_engine::intent::definition::{Condition, TargetKind, TargetSpec};
use oryn_engine::intent::verifier::{Verifier, VerifierContext};
...
async fn test_verify_pattern_exists() {
    let patterns = DetectedPatterns {
        login: Some(oryn_engine::protocol::LoginPattern {
            email: None,
            username: None,
            password: 0,
            submit: None,
