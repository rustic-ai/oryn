...
use oryn_common::command::{Command, CookieAction, TabAction, Target};
use oryn_common::protocol::{
    Cookie, ScanResult, ScannerData, ScannerProtocolResponse, ScannerRequest,
};
use oryn_common::translator::{TranslationError, translate};
    ...
    pub async fn execute_command<B: Backend>(
        ...
        match &cmd {
            ...
            Command::Intents(filter) => {
                use oryn_common::command::IntentFilter;
            ...
            Command::Learn(action) => {
                use oryn_common::command::LearnAction;
                ...
                let resolved_cmd = if let Some(scan) = &self.last_scan {
                    match ResolutionEngine::resolve(cmd.clone(), scan, backend).await {
                        ...
                        Err(_) => {
                            ...
                            let req =
                                ScannerRequest::Scan(oryn_common::protocol::ScanRequest::default());
                    ...
                    let req = ScannerRequest::Scan(oryn_common::protocol::ScanRequest::default());
                    let resp = backend.execute_scanner(req).await?;
                    self.update_from_response(&resp);
