...
pub fn format_response(resp: &ScannerProtocolResponse) -> String {
    ...
    match resp {
        ScannerProtocolResponse::Ok { data, .. } => match data.as_ref() {
            ScannerData::Scan(scan) => {
                ...
                // scan.patterns is available.
                 if let Some(patterns) = &scan.patterns {
                    ...
                    if patterns.login.is_some() { detected.push("Login Form"); }
                    if patterns.search.is_some() { detected.push("Search Box"); }
                    if patterns.pagination.is_some() { detected.push("Pagination"); }
                    if patterns.modal.is_some() { detected.push("Modal"); }
                    if patterns.cookie_banner.is_some() { detected.push("Cookie Banner"); }
            ...
            ScannerData::Action(a) => format!("Action Result: success={}, msg={:?}", a.success, a.message),
...
pub fn mask_sensitive(value: &str, field_name: &str, sensitive_fields: &[String]) -> String {
    let default_sensitive = [
        "password", "secret", "token", "key", "cvv", "ssn", "card_number",
    ];
