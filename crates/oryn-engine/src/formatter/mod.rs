                    ...
                    if !intents.is_empty() {
                        ...
                        sorted.sort_by(|a, b| {
                            ...
                            let status_ord = match (&a.status, &b.status) {
                                (
                                    oryn_common::protocol::AvailabilityStatus::Ready,
                                    oryn_common::protocol::AvailabilityStatus::Ready,
                                ) => std::cmp::Ordering::Equal,
                                (oryn_common::protocol::AvailabilityStatus::Ready, _) => {
                                    std::cmp::Ordering::Less
                                }
                                (_, oryn_common::protocol::AvailabilityStatus::Ready) => {
                        ...
                        for intent in sorted {
                            let status_icon = match intent.status {
                                oryn_common::protocol::AvailabilityStatus::Ready => "🟢",
                                oryn_common::protocol::AvailabilityStatus::NavigateRequired => "🟠",
                                oryn_common::protocol::AvailabilityStatus::MissingPattern => "🔴",
                                oryn_common::protocol::AvailabilityStatus::Unavailable => "⚫",
                            };
