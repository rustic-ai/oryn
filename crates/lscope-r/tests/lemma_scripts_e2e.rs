...
async fn test_all_lemma_scripts_remote() {
    ...
    // Path resolution check: root is crates/lscope-r
...
async fn execute_test_command(
    ...
    if let lscope_core::protocol::ScannerProtocolResponse::Ok { data, .. } = &resp {
        if let lscope_core::protocol::ScannerData::Scan(scan_result) = data.as_ref() {
            state.resolver_context = Some(ResolverContext::new(scan_result));
        }
    } else if let lscope_core::protocol::ScannerProtocolResponse::Error { message, .. } = resp {
