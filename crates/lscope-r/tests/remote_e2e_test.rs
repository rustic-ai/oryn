...
async fn test_remote_extension_e2e() {
    ...
    if let Err(e) = page.goto(url).await {
        println!("Warning: Failed to navigate to {}, falling back to data URL. Error: {}", url, e);
    ...
    println!("STEP 6: Waiting for extension connection to port {}...", port);
    ...
    match resp_result {
        ...
        Err(_) => {
            ...
            panic!("Timeout waiting for extension response. Extension failed to connect to port {}.", port);
        }
    }
