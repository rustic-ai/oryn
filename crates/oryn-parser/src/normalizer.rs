...
lazy_static! {
    static ref SHORT_OPTION: Regex = Regex::new(r"(^|\s)-([a-zA-Z0-9]+)(?=\s|$)").unwrap();
}
    ...
    while let Some(arg) = arg_iter.next() {
        ...
        if arg.starts_with("--") {
            ...
            let norm = match verb.as_str() {
                "observe" => {
                    if arg.eq_ignore_ascii_case("full") { "--full".to_string() }
                    else if arg.eq_ignore_ascii_case("minimal") { "--minimal".to_string() }
                    else { arg }
                }
                "press" => {
                    if arg.eq_ignore_ascii_case("control") { "control".to_string() } // keep key names
                    else if arg.eq_ignore_ascii_case("shift") { "shift".to_string() }
                    else if arg.eq_ignore_ascii_case("alt") { "alt".to_string() }
                    else if arg.eq_ignore_ascii_case("meta") { "meta".to_string() }
                        ...
                        arg.to_lowercase()
                         ...
                         match lower.as_str() {
                             ...
                             "f1" | "f2" | "f3" | "f4" | "f5" | "f6" | "f7" | "f8" | "f9" | "f10" | "f11" | "f12" => lower,
       ...
       // Only one block needed
        ...
        if let Some(idx) = string_idx {
            ...
            let has_relational_after = normalized_args.iter().skip(idx + 1).any(|arg| {
                let lower = arg.to_lowercase();
                matches!(lower.as_str(), "inside" | "near" | "after" | "before" | "contains")
            });
