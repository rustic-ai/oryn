use oryn_common::protocol::{
    Action, Element, ElementState, PageInfo, Rect, ScanResult, ScanStats, ScannerAction,
...
fn test_process_multiple() {
    ...
    match &actions[0] {
        Action::Scanner(ScannerAction::Click(req)) => assert_eq!(req.id, Some(1)),
    ...
    match &actions[1] {
        Action::Scanner(ScannerAction::Wait(req)) => {
            ...
            assert_eq!(req.text.as_deref(), Some("Submit"));
        }
