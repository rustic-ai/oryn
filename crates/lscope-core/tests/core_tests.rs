...
use lscope_core::formatter;
use lscope_core::protocol::{
...
fn test_protocol_serialization() {
    ...
    let req = ScannerRequest::Click(lscope_core::protocol::ClickRequest {
        id: 42,
        button: lscope_core::protocol::MouseButton::Left,
        double: false,
    ...
    let req = ScannerRequest::Scan(lscope_core::protocol::ScanRequest::default());
    let json = serde_json::to_string(&req).unwrap();
...
fn test_translator_click() {
    ...
    assert!(err.is_err());
    match err {
        Err(lscope_core::translator::TranslationError::InvalidTarget(_)) => {}
...
fn test_formatter_ok() {
    let result = ScannerData::Action(lscope_core::protocol::ActionResult {
        success: true,
