...
async fn test_full_flow_search() {
    ...
    let chrome_path = std::env::var("CHROME_BIN").unwrap_or_else(|_| "/usr/bin/google-chrome".to_string());
    ...
    match backend.launch().await {
        Ok(_) => {
            ...
            if let lscope_core::command::Command::GoTo(url) = &commands[0] {
            ...
            let scan_cmd = lscope_core::command::Command::Observe(std::collections::HashMap::new());
            let scan_req = translate(&scan_cmd).expect("Failed to translate scan");
            
            let scan_res = backend.execute_scanner(scan_req).await.expect("Scan failed");
            
            // 5. Find an element to click (e.g., h1)
            let mut target_id = None;
            if let lscope_core::protocol::ScannerProtocolResponse::Ok { data, .. } = scan_res {
                if let lscope_core::protocol::ScannerData::ScanValidation(scan_data) = data {
            ...
            if let Some(id) = target_id {
                let click_cmd = lscope_core::command::Command::Click(
                    lscope_core::command::Target::Id(id as usize), 
                    std::collections::HashMap::new()
                );
                let click_req = translate(&click_cmd).expect("Failed to translate click");
                let click_res = backend.execute_scanner(click_req).await.expect("Click failed");
...
async fn test_error_handling() {
    ...
    let click_cmd = lscope_core::command::Command::Click(
        lscope_core::command::Target::Id(999999), 
        std::collections::HashMap::new()
    );
    let click_req = translate(&click_cmd).expect("Translation failed");
    
    // navigate first to have a context
    backend.navigate("data:text/html,<html><body></body></html>").await.ok();
    
    let res = backend.execute_scanner(click_req).await;
    match res {
        Ok(lscope_core::protocol::ScannerProtocolResponse::Error { code, .. }) => {
...
async fn test_interaction_type() {
    ...
    backend.navigate("data:text/html,<html><body><input id='inp' type='text'></body></html>").await.expect("Nav failed");

    // Scan to find ID
    let scan_req = translate(&lscope_core::command::Command::Observe(std::collections::HashMap::new())).unwrap();
    ...
    if let Some(id) = input_id {
        let type_cmd = lscope_core::command::Command::Type(
            lscope_core::command::Target::Id(id as usize), 
        ...
        match type_res {
            lscope_core::protocol::ScannerProtocolResponse::Ok { .. } => println!("Type success"),
