...
fn test_vectors_compliance() {
    ...
    let groups: HashMap<String, TestGroup> =
    ...
    for (group_name, group) in groups {
        ...
        let _ = &group.metadata.description;
        let _ = &group.metadata.version;
        for vector in group.vectors {
            ...
            if let Some(expected_canonical) = &vector.canonical {
                    ...
                    Ok(_) => {
                        if let Some(err_exp) = &vector.error {
                            if err_exp.phase == "parsing" {
                                let msg = format!(
                                    "Expected parsing error for {}, but succeeded",
                                    vector.id
                                );
                                println!("    [FAIL] {}", msg);
                                failures.push(msg);
                ...
                if err_exp.phase == "parsing" && parse(&normalized).is_ok() {
