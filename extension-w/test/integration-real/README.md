...
- Default PR workflow (`.github/workflows/ci-js.yml`) runs `npm run test:all` and does not include `test:integration:real`.
- Keep these real WASM tests as a manual pre-release check, especially after Rust/WASM changes.
